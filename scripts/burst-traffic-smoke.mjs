...
function loadFreshModules() {
  ...
  const pharmacyModulePath = require.resolve("../api/_lib/pharmacy-search");
  const openFdaModulePath = require.resolve("../api/_lib/openfda");
  ...
  return {
    ...
    pharmacy: require("../api/_lib/pharmacy-search"),
    openFda: require("../api/_lib/openfda"),
  };
}
...
async function runDrugBurst() {
  ...
  const candidates = Array.from({ length: 4 }, (_, index) => ({
    ...
    genericNames: [`Generic ${index + 1}`],
    applicationNumbers: [`ANDA${index + 1}`],
    productNdcs: [`12345-000${index + 1}`],
  }));
  ...
  const metrics = await withMockedFetch(async (url) => {
    ...
    try {
      ...
      if (requestUrl.pathname.endsWith("/drug/ndc.json")) {
        return createJsonResponse({
          meta: { results: { total: 1, skip: 0, limit: 60 } },
          results: [{ product_ndc: "12345-0001" }],
        });
      }

      if (requestUrl.pathname.endsWith("/drug/drugsfda.json")) {
        return createJsonResponse({
          meta: { results: { total: 1, skip: 0, limit: 20 } },
          results: [{ application_number: "ANDA1", products: [{ brand_name: "Brand 1" }] }],
        });
      }

      if (
        requestUrl.pathname.endsWith("/drug/shortages.json") ||
        requestUrl.pathname.endsWith("/drug/enforcement.json")
      ) {
        return createJsonResponse({
      ...
      throw new Error(`Unexpected openFDA URL: ${requestUrl.toString()}`);
    } finally {
      record(key, -1);
    }
  }, async () => {
    ...
    const requests = Array.from({ length: 8 }, async () => {
      await Promise.all([
        ...
        openFda.searchDrugApplications(["adderall xr 20 mg", "adderall"]),
      ]);

      await Promise.all(
        candidates.map(async (candidate) => {
          await Promise.allSettled([
            openFda.searchShortagesForCandidate(candidate),
            openFda.searchRecallsForCandidate(candidate),
          ]);
        }),
      );
    });
  ...
  return {
    ...
    maxConcurrentPerRequest: Math.max(...maxConcurrent.values()),
  };
}
...
async function runOpenFdaStaleFallback() {
  ...
  const recovered = await withMockedNow(
    ...
    () =>
      ...
      }, async () => openFda.searchNdcRecords(["adderall xr 20 mg", "adderall"])),
  );
