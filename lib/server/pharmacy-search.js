...
const MEDICATION_PROFILES = {
  controlled_stimulant: {
    label: "Controlled medication search",
    workflow_label: "Call before transfer",
  ...
  acute_antibiotic: {
    label: "Same-day treatment search",
    workflow_label: "Fast pickup check",
  ...
  cold_chain: {
    label: "Cold-chain medication search",
    workflow_label: "Shipment timing check",
  ...
  maintenance_refill: {
    label: "Routine refill search",
    workflow_label: "Routine refill check",
...
function normalizeAddressKey(value) {
  return sanitizeText(value)
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/\b(apartment|apt|suite|ste|unit|floor|fl)\s+[a-z0-9-]+\b/g, " ")
...
function normalizePharmacyNameKey(value) {
  const normalized = sanitizeText(value)
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/\b(pharmacy|drugs?|drugstore|apothecary|health|rx|store|location)\b/g, " ")
  ...
  if (/\b(walgreens|duane reade)\b/.test(normalized)) {
    return "walgreens";
  }

  if (/\bcvs\b/.test(normalized)) {
    return "cvs";
  }

  if (/\brite aid\b/.test(normalized)) {
    return "rite aid";
  }

  if (/\bcostco\b/.test(normalized)) {
    return "costco";
  }

  if (/\bwalmart\b/.test(normalized)) {
    return "walmart";
  }

  if (/\bpublix\b/.test(normalized)) {
    return "publix";
  }

  if (/\bkroger\b/.test(normalized)) {
    return "kroger";
  }
...
function buildCoordinateKey(coordinates) {
  if (!coordinates || !Number.isFinite(coordinates.lat) || !Number.isFinite(coordinates.lng)) {
    return "";
  }
...
function buildPlaceDedupeKeys(place) {
  const keys = [];
  const addressKey = normalizeAddressKey(place.address);
  const nameKey = normalizePharmacyNameKey(place.name);
  const phoneKey = normalizePhoneKey(
    place.international_phone_number || place.phone_number || place.phone_link,
  );
  const coordinateKey = buildCoordinateKey(place.coordinates);

  if (place.place_id) {
    keys.push(`place:${place.place_id}`);
  }

  if (addressKey && nameKey) {
    keys.push(`name:${nameKey}::${addressKey}`);
  }

  if (addressKey && phoneKey) {
    keys.push(`phone:${phoneKey}::${addressKey}`);
  }

  if (coordinateKey && nameKey) {
    keys.push(`coord:${nameKey}::${coordinateKey}`);
  }

  return keys;
}

function getDuplicatePreferenceScore(place) {
  const openScore =
    place.open_now === true ? 32 : place.open_now === false ? -10 : 0;
  const ratingScore = Number.isFinite(place.rating) ? place.rating * 10 : 0;
  const reviewScore = Number.isFinite(place.user_ratings_total)
    ? Math.min(place.user_ratings_total, 250) * 0.08
    : 0;
  const distanceScore = Number.isFinite(place.distance_miles)
    ? Math.max(0, 16 - place.distance_miles)
    : 0;
  const phoneScore =
    place.phone_number || place.international_phone_number || place.phone_link
      ? 5
      : 0;

  return openScore + ratingScore + reviewScore + distanceScore + phoneScore;
}

function dedupeNearbyPlaceResults(results) {
  const deduped = [];
  ...
  for (const place of results) {
    ...
    if (existingIndex === undefined) {
      const nextIndex = deduped.push(place) - 1;
      dedupeKeys.forEach((key) => keyToIndex.set(key, nextIndex));
      continue;
    }

    const resolvedIndex = keyToIndex.get(existingIndex);
    const currentPlace = deduped[resolvedIndex];
    const keepIncoming =
      !currentPlace ||
      getDuplicatePreferenceScore(place) > getDuplicatePreferenceScore(currentPlace);

    if (keepIncoming) {
      deduped[resolvedIndex] = {
        ...currentPlace,
        ...place,
      };
    }

    dedupeKeys.forEach((key) => keyToIndex.set(key, resolvedIndex));
    buildPlaceDedupeKeys(deduped[resolvedIndex]).forEach((key) =>
      keyToIndex.set(key, resolvedIndex),
    );
  }
...
function buildMedicationGuidance(medicationProfile, medication) {
  if (medicationProfile.key === "controlled_stimulant") {
    return {
      ...
      summary: `${medication} often takes more calls to place, so the first option favors pharmacies that are open, well reviewed, and still close enough to try quickly.`,
      ranking_focus:
        "Best overall match balances open hours, review depth, and distance so the first call is practical.",
      ranking_focus_label: medicationProfile.ranking_focus_label,
      recommended_action: `Ask whether ${withStrengthQualifier(medication)} is available today before you transfer or send the prescription.`,
      tags: ["Controlled medication", "Call before transfer", "Stock still unconfirmed"],
      real_signal: REAL_RESULTS_LABEL,
      demo_boundary:
        "This guidance helps with call order only. PharmaPath is not verifying controlled-substance stock in real time.",
    };
  }
  ...
  if (medicationProfile.key === "acute_antibiotic") {
    return {
      ...
      summary: `${medication} is treated like a same-day search, so the list leans harder toward nearby pharmacies that are open now.`,
      ranking_focus:
        "Best overall match leans on distance and open-now status to support a faster pickup path.",
      ranking_focus_label: medicationProfile.ranking_focus_label,
      recommended_action: `Ask how quickly ${medication} can be prepared and what the pickup window looks like today.`,
      tags: ["Same-day medication", "Closest open options", "Stock still unconfirmed"],
      real_signal: REAL_RESULTS_LABEL,
      demo_boundary:
        "This guidance reflects a same-day call strategy only. PharmaPath is not confirming antibiotic inventory live.",
    };
  }
  ...
  if (medicationProfile.key === "cold_chain") {
    return {
      ...
      summary: `${medication} often depends on dose timing and shipment schedules, so the list leans toward dependable pharmacies that are still practical to call quickly.`,
      ranking_focus:
        "Best overall match balances reputation, review volume, current hours, and travel distance for medications that often need shipment or refrigeration questions.",
      ranking_focus_label: medicationProfile.ranking_focus_label,
      ...
      tags: ["Shipment-sensitive", "Dose-specific call", "Stock still unconfirmed"],
      real_signal: REAL_RESULTS_LABEL,
      demo_boundary:
        "This guidance helps frame the refill conversation. PharmaPath is not reading live refrigerated-inventory feeds.",
    };
  }

  return {
    title: "Routine refill guidance",
    summary: `${medication} is treated as a routine refill search, so the list balances nearby access, pharmacy reputation, and current hours for the first call.`,
    ranking_focus:
      "Best overall match balances proximity, review quality, and current hours to make the first call easier to explain.",
    ranking_focus_label: medicationProfile.ranking_focus_label,
    recommended_action: `Ask about refill turnaround, transfer timing, and insurance processing for ${medication}.`,
    tags: ["Routine refill", "Balanced call list", "Stock still unconfirmed"],
    real_signal: REAL_RESULTS_LABEL,
    demo_boundary:
        "This guidance supports call order only. PharmaPath is not verifying routine-medication stock in real time.",
  };
}
...
function buildPlaceWorkflow(place, medicationProfile, medication) {
  ...
  if (medicationProfile.key === "controlled_stimulant") {
    const matchReason =
      Number.isFinite(place.user_ratings_total) && place.user_ratings_total >= 100
        ? "Well-reviewed option for the first call."
        : distanceBand === "very_close"
          ? "Very close option if you want to check availability today."
          : "Good backup if the first pharmacy cannot help.";

    return {
      workflow_label: medicationProfile.workflow_label,
      match_reason: matchReason,
      inventory_note: `${place.name} is a real nearby result, but ${medication} stock is not confirmed live in PharmaPath.`,
    };
  }
  ...
  if (medicationProfile.key === "acute_antibiotic") {
    const matchReason =
      distanceBand === "very_close"
        ...
        : distanceBand === "nearby"
          ? "Still close enough to try early."
  ...
  if (medicationProfile.key === "cold_chain") {
    const matchReason =
      Number.isFinite(place.rating) && place.rating >= 4.7
        ? "Higher-rated option for a medication that often depends on shipment timing."
        : "Reasonable backup for confirming dose-specific availability and delivery timing.";
  ...
  return {
    ...
    match_reason:
      distanceBand === "very_close"
        ? "Convenient nearby option for a routine refill."
...
async function fetchJson(url) {
  return googleRequestCache.getOrLoad(url.toString(), async () => {
    const activeGoogleApiBackoff = getGoogleApiBackoffState();
    if (activeGoogleApiBackoff) {
      throw createGoogleApiError(
        activeGoogleApiBackoff.message || "Location service is temporarily unavailable.",
        503,
        activeGoogleApiBackoff.code || "google_api_backoff",
      );
    }
    ...
    try {
      ...
      if (status === "OVER_QUERY_LIMIT" || status === "REQUEST_DENIED") {
        const activeBackoff = activateGoogleApiBackoff({
          ...
          message: payload?.error_message || `Location service failed with status ${status}.`,
          durationMs:
        ...
        throw createGoogleApiError(
          activeBackoff.message || payload?.error_message || `Location service failed with status ${status}.`,
          503,
          activeBackoff.code || status.toLowerCase(),
        );
      }
...
async function searchNearbyPharmacies({
  ...
  const normalizedResults = Array.isArray(payload.results)
    ...
    : [];
  const dedupedResults = dedupeNearbyPlaceResults(normalizedResults);

  const sortedResults = dedupeNearbyPlaceResults(
    await Promise.all(
      sortResults(dedupedResults, sortBy, medicationProfile)
      .slice(0, MAX_RESULTS)
      .map((place) =>
