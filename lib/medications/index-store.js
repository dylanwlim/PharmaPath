...
const PUBLIC_SNAPSHOT_PATH = "/medication-index.snapshot.json.gz";
...
async function gunzipToText(rawValue) {
  if (typeof DecompressionStream === "function") {
    const stream = new Response(rawValue).body;

    if (!stream) {
      throw new Error("Unable to read the medication snapshot stream.");
    }

    return new Response(stream.pipeThrough(new DecompressionStream("gzip"))).text();
  }

  return zlib.gunzipSync(Buffer.from(rawValue)).toString("utf8");
}

async function loadSnapshotFromAsset(snapshotUrl) {
  ...
  if (!response.ok) {
    throw new Error(
      `Unable to load the medication index asset (${response.status} ${response.statusText}).`,
    );
  }

  const serialized = await gunzipToText(await response.arrayBuffer());
  return JSON.parse(serialized);
}

function normalizeSnapshotUrl(snapshotUrl) {
  return sanitizeText(snapshotUrl) || null;
}

function getSnapshotSourceKey(snapshotUrl) {
  return normalizeSnapshotUrl(snapshotUrl) || SNAPSHOT_PATH;
}

async function loadMedicationSnapshot(snapshotUrl) {
  const normalizedSnapshotUrl = normalizeSnapshotUrl(snapshotUrl);
  ...
  try {
    return await loadSnapshotFromDisk();
  } catch {
    return loadSnapshotFromAsset(normalizedSnapshotUrl);
  }
}

function getMedicationSnapshot({ snapshotUrl } = {}) {
  const sourceKey = getSnapshotSourceKey(snapshotUrl);

  if (!snapshotPromises.has(sourceKey)) {
    snapshotPromises.set(
      sourceKey,
      loadMedicationSnapshot(snapshotUrl).catch((error) => {
        snapshotPromises.delete(sourceKey);
        throw error;
      }),
    );
  }

  return snapshotPromises.get(sourceKey);
}
...
function getPreparedMedicationIndex({ snapshotUrl } = {}) {
  const sourceKey = getSnapshotSourceKey(snapshotUrl);

  if (!preparedIndexPromises.has(sourceKey)) {
    preparedIndexPromises.set(
      sourceKey,
      getMedicationSnapshot({ snapshotUrl })
        .then((snapshot) => prepareMedicationIndex(snapshot))
        .catch((error) => {
          preparedIndexPromises.delete(sourceKey);
          throw error;
        }),
    );
  }

  return preparedIndexPromises.get(sourceKey);
}

function searchMedicationOptions(
  query,
  { limit = DEFAULT_SEARCH_LIMIT, exact = false, snapshotUrl } = {},
) {
  return getPreparedMedicationIndex({ snapshotUrl }).then((prepared) => {
...
function resolveMedicationOption(query, { snapshotUrl } = {}) {
  return getPreparedMedicationIndex({ snapshotUrl }).then((prepared) => {
    const normalizedQuery = normalizeMedicationText(query);
    const normalizedIdentifierQuery = normalizeIdentifier(query);
...
function resolveMedicationProfile(query, { snapshotUrl } = {}) {
  return resolveMedicationOption(query, { snapshotUrl }).then((exactOption) => {
    if (exactOption) {
      return {
...
function buildRuntimeSnapshotUrl(requestUrl) {
  const origin = typeof requestUrl === "string" ? new URL(requestUrl).origin : "";

  return origin ? `${origin}${PUBLIC_SNAPSHOT_PATH}` : null;
}
