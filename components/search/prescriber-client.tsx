...
export function PrescriberClient({
  ...
  useEffect(() => {
    ...
    async function loadMedicationEvidence() {
      try {
        ...
        setPayload(null);
        setError(reason instanceof Error ? reason.message : "Unable to load medication data.");
        setLastCompletedQuery(query);
      }
    }
  ...
  const patientFinderHref = location
    ...
    : "/patient";
  const overviewMetrics = selectedMatch
    ? [
        ...
        <div className="site-shell grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">
              Keep shortage pressure, manufacturer breadth, formulation coverage, and recall
              context visible while you decide how much contingency planning the prescription needs.
            </p>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                  <section className="surface-panel rounded-[2rem] p-6">
                    ...
                    <p className="mt-4 text-sm leading-6 text-slate-700">{routingRecommendation}</p>
                ...
                <section className="rounded-[2rem] border border-violet-200 bg-violet-50 p-6">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                      <span className="eyebrow-label text-violet-700">Patient handoff</span>
                      <p className="mt-3 max-w-xl text-sm leading-6 text-violet-950">
                        Carry this medication family into Pharmacy Finder when you need nearby
                        routing in parallel with the evidence summary above.
                      </p>
                    </div>
                    <NextLink href={patientFinderHref} className="action-button-secondary text-sm">
